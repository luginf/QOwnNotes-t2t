...
Vous pouvez utiliser votre propre format d'horodatage lors de l'insertion de la date et l'heure actuelle dans une note.

Vous pouvez utiliser cette page pour personnaliser le format d'horodatage par défaut. Vous pouvez entrer cette chaîne dans _Note_ > _Préférences_ > _Éditeur_ > _Insertion de la date et de l'heure courantes_.
...
| `MMM`      | le nom abrégé du mois dans la langue configurée (par exemple `jan.` à `déc.`).        |
| `MMMM`     | le long nom du mois dans la langue configurée (par exemple `janvier` à `décembre`).   |
...
| `zzz`       | les millisecondes avec des zéros non significatifs (000 à 999)                  |
| `AP` ou `A` | utiliser l'affichage AM/PM. A/AP sera remplacé par `AM` ou `PM`.                |
| `ap` ou `a` | utiliser l'affichage am/pm. a/ap sera remplacé par `am` ou `pm`.                |
| `t`         | le fuseau horaire (par exemple `CEST`)                                          |

## Exemple de chaînes formatées
...
::: tip
Si vous devez effectuer des opérations plus complexes pour afficher la date et l'heure courantes, envisagez de créer une [action personnalisée](../scripting/methods-and-objects.md#registering-a-custom-action).
:::
